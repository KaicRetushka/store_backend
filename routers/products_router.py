...
@products_router.get("/me", tags=["Товары (Products)"], dependencies=[Depends(security.access_token_required)],
                     description="Возращает все товары пользователя, которые он создал")
...
@products_router.get("/", tags=["Товары (Products)"], description="Возращает все товары")
async def give_products() -> List[ProductInfoSchema]:
    products_list = await select_products()
    return products_list

@products_router.get("/{id}", tags=["Товары (Products)"], description="Возращает товар по определенному id")
...
@products_router.post("/", tags=["Товары (Products)"],
                      dependencies=[Depends(security.access_token_required)], description="Создает товар")
...
@products_router.delete("/{id}", tags=["Товары (Products)"],
                        dependencies=[Depends(security.access_token_required)], description="Удаляет товар")
